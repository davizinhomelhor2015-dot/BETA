...
async function main() {
  ...
  for (const [setor, cfg] of Object.entries(cfgs || {})) {
    ...
    // backup (dataRefFim, normalmente ontem) — NUNCA inclui o dia de hoje,
    // porque o dia de hoje pertence ao PRÓPRIO backup de amanhã, que vai
    // cobri-lo corretamente. Fotos de dias anteriores ao início do bloco
    // (sobras de backups que falharam) também entram, mas marcadas como
    // "atrasada" — assim nenhuma foto se perde, mas nada do dia de hoje é
    // erroneamente rotulado como atrasado.
